'''
Use some runplays, include that play chart
drive1( 0 = field goal, 1 = td attempt)
 time for play function
...
# Now we have 2 results let us analyze them:
    if ('INT' in str(oResult) or 'INT' in str(dResult)) and ('(' not in str(dResult)):
        ...
        if 'F' in str(oQT):
            #FIXME need to include a different thing here
            return 'LOSE'
        else:
            dRoll2 = dice.dDice()-1
        ...
        #print('incomplete')
        return 0
    elif 'TD' in str(oResult) or 'TD' in str(dResult):
        return 'TD'
    else:
        #print('normal')
        #print(oResult,dResult)
...
def updateDrive(play_result,down, yards_to_first, distance, time):
    ...
    #alter the timing to be a bit more realistic- 
    return down,yards_to_first,distance,time

def fullDrive(down,yards_to_first,distance,time):
    while (time>0) and (distance>0) and (down <5):
        play1 = play()
        if "TD" in str(play1):
            down,yards_to_first,distance,time = updateDrive(play1,down,yards_to_first,distance,time)
            #print('TD')
            return 1,time
            break
        elif 'LOSE' in str(play1):
            #print('LOSE')
            down,yards_to_first,distance,time = updateDrive(play1,down,yards_to_first,distance,time)
            return 0,time
            break
...
if __name__ == "__main__":
    ...
    '''
    Now I will try to simulate a single drive
    '''
    while (time>0) and (distance>0) and (down <5):
        play1 = play()
        if "TD" in str(play1):
